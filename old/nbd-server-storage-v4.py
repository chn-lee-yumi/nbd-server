...
def serve_client(conn, addr):
    ...
    if method == b'R':
        ...
        while len(write_data) < length:
            more = conn.recv(length - len(write_data))
            write_data += more
...
if __name__ == '__main__':
    ...
    sock.listen(1000)  # 这个需要大一点，可能回收需要时间，如果改成个位数，会导致write的时候一卡一卡（read不会），在客户端connect的时候卡1秒，不知道为什么
    """
    内核调优：
