"""
nbd server, remote storage, via raw socket
"""
import socket
import struct
import threading
...
class NbdServer:
    ...
    def read(self, offset, length):
        ...
        if offset % BLOCK_SIZE + length > BLOCK_SIZE:
            ...
            content = remote_read(server=node, block=block_number, offset=offset % BLOCK_SIZE, length=BLOCK_SIZE - offset % BLOCK_SIZE)
            block_number += 1
            node = STORAGE_NODES[block_number % STORAGE_NODE_COUNT]
            content += remote_read(server=node, block=block_number, offset=0, length=length - (BLOCK_SIZE - offset % BLOCK_SIZE))
        else:
            content = remote_read(server=node, block=block_number, offset=offset % BLOCK_SIZE, length=length)
    ...
    def write(self, offset, data):
        ...
        if offset % BLOCK_SIZE + len(data) > BLOCK_SIZE:
            ...
            remote_write(server=node, block=block_number, offset=offset % BLOCK_SIZE, data=tmp_data)
            block_number += 1
            node = STORAGE_NODES[block_number % STORAGE_NODE_COUNT]
            tmp_data = data[slice(BLOCK_SIZE - offset % BLOCK_SIZE, len(data))]
            remote_write(server=node, block=block_number, offset=0, data=tmp_data)
        else:
            remote_write(server=node, block=block_number, offset=offset % BLOCK_SIZE, data=data)


def remote_read(server="", block=0, offset=0, length=0):
    if DEBUG:
        print("remote_read", block, offset, length)
    s = socket.socket()
    host, port = server.split(":")
    # t0 = time.time()
    s.connect((host, int(port)))
    # t1 = time.time()
    # print("read connect time: %.5f" % (t1 - t0))
    s.send(struct.pack("!sLQL", b"R", block, offset, length))
    _s = BuffSock(s)
    data = _s.recv(length)
    s.close()
    return data
...
def remote_write(server="", block=0, offset=0, data=b''):
    # TODO: 排查有时候connect很慢的问题
    s = socket.socket()
    host, port = server.split(":")
    # t0 = time.time()
    s.connect((host, int(port)))
    # t1 = time.time()
    # print("write connect time: %.5f" % (t1 - t0))
    s.send(struct.pack("!sLQL", b"W", block, offset, len(data)))
    s.send(data)
    s.close()
    return
